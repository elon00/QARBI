...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const tabs: { key: TabKey; label: string; icon: React.FC<{ className?: string }>; badge?: string | number; badgeColor?: string }[] = [
    {
      key: "spawner",
      label: t?.tabs?.spawner || "Agent Citadel",
      icon: Users,
      badge: agentCount,
    ...
    {
      key: "conway",
      label: t?.tabs?.conway || "Conway State Engine",
      icon: Grid,
      badge: "Stylus Rust",
    ...
    {
      key: "terminal",
      label: t?.tabs?.terminal || "Agentic AI Dispatcher",
      icon: Terminal,
      badge: "AI Powered",
      badgeColor: "bg-sky-900 text-sky-300 border-sky-700",
    },
    {
      key: "tasks",
      label: t?.tabs?.tasks || "Agent Economy & Market",
      icon: Layers,
      badge: openTaskCount,
    ...
    {
      key: "security",
      label: t?.tabs?.security || "Policy & Security Enclave",
      icon: ShieldCheck,
    ...
    {
      key: "explorer",
      label: t?.tabs?.explorer || "Arbitrum Ledger",
      icon: Activity,
    },
    {
      key: "whitepaper",
      label: t?.tabs?.whitepaper || "Technical Whitepaper",
      icon: FileText,
    },
  ];
  ...
  return (
    <nav className="bg-slate-900/80 border-b border-slate-800 backdrop-blur sticky top-20 z-30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex space-x-2 overflow-x-auto py-2.5 scrollbar-thin scrollbar-thumb-slate-700">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-3.5 py-2 text-xs font-semibold rounded-lg whitespace-nowrap transition cursor-pointer ${
                  isActive
              ...
              >
                ...
                {tab.badge !== undefined && (
                  <span
                    className={`ml-1 px-1.5 py-0.5 text-[10px] font-mono rounded-full border ${
                      isActive ? "bg-white/20 text-white border-white/30" : tab.badgeColor || "bg-slate-800 text-slate-400 border-slate-700"
                    }`}
                  >
                    {tab.badge}
                  </span>
