...
// SOLIDITY COMPILATION & ARTIFACT SYNCHRONIZATION
command("solidity contracts compilation", "npm", ["run", "compile:contracts"], { timeout: 180000 });
command("contract artifacts synchronization", "npm", ["run", "generate:artifacts"], { timeout: 180000 });

// TEST / EXECUTION
command("NIST PQC pure lattice tests", "npm", ["run", "test:nist"], { timeout: 180000 });
command("quantum portfolio & inter-agent channel", "npm", ["run", "test:quantum"], { timeout: 180000 });
command("URS 10-gate universal reality audit", "npm", ["run", "reality:universal"], { timeout: 180000 });
command("QARBI canonical finish hygiene", "npm", ["run", "check:repo"], { timeout: 180000 });
...
const deployUrl = process.env.QMOOSA_DEPLOYMENT_URL || "https://elon00.github.io/QARBI/";
...
try {
  ...
  const hasAppDom = html.includes('id="root"') || html.includes('assets/index') || html.includes('QARBI');
